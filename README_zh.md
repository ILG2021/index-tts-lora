...
`metadata.csv` 是 UTF-8 编码、竖线分隔的三列（不是逗号分隔）：

```text
LJ001-0001|原始文本|规范化后的朗读文本
LJ001-0002|原始文本|规范化后的朗读文本
```

ID 不带 `.wav` 后缀，不能包含路径分隔符，Windows 下大小写也不能重复。默认使用第三列，音频必须存在于 `wavs`。转换器用标准库读取 WAV，建议输入 PCM WAV；不要只给 MP3 改扩展名。空文本会跳过。自定义音色数据必须让音频与文本逐条对应，避免截断、背景音乐及多说话人混合。
