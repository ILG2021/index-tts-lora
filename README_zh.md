...
转换器会直接在 `wavs` 中查找第一栏给出的文件，不会再次追加后缀；生成的样本 ID 会去掉 `.wav`。文件名不能包含路径分隔符，Windows 下去掉后缀后也不能大小写重复。音频建议为真正的 PCM WAV，不要只给 MP3 改扩展名。空文本会跳过。自定义音色数据必须让音频与文本逐条对应，避免截断、背景音乐及多说话人混合。
