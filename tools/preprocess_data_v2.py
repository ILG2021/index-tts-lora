...
Adapted from instavar/indextts2-finetuning. Uses the official IndexTTS2
normalizer; --language labels the dataset and does not add language support.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        default=Path("datasets/ljspeech.jsonl"),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "`en=datasets/ljspeech.jsonl` or "
            "`en=datasets/ljspeech.jsonl=processed_data/ljspeech`."
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Feature extraction batch size; currently only 1 is supported.",
    )
    parser.add_argument(
        "--workers",
        type=int,
...
class SemanticExtractor:
    def __init__(self, stats_path: Path, model_path: Path, device: torch.device):
        self.device = device
        self.feature_extractor = SeamlessM4TFeatureExtractor.from_pretrained(
            str(model_path), local_files_only=True
        )
        self.semantic_model, self.semantic_mean, self.semantic_std = build_semantic_model(
            path_=stats_path, model_path=str(model_path), local_files_only=True
        )
        ...
        with open(manifest_path, "r", encoding="utf-8") as source:
            for line in tqdm(source, desc=f"Preprocessing [{dataset_language}]", unit="sample"):
                ...
                if args.skip_existing and (
                    uid in (train_ids | val_ids)
                    and (dirs["condition"] / f"{uid}.npy").exists()
                    and (dirs["emo"] / f"{uid}.npy").exists()
                    and
...
def main() -> None:
    ...
    if device.type == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("CUDA requested but unavailable. Check the PyTorch installation and NVIDIA driver.")
    ...
    stats_path = Path(stats_value or "wav2vec2bert_stats.pt")
    ...
    semantic_code_ckpt = args.config.parent / "hf_cache" / "semantic_codec_model.safetensors"
    if not semantic_code_ckpt.is_file():
        semantic_code_ckpt = Path(hf_hub_download(
