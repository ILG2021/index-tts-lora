...
def hf_endpoint() -> str:
    ...
    Honors the standard HF_ENDPOINT environment variable. Falls back to
...
def ms_endpoint() -> str:
    ...
    This vendored source does not provide a default endpoint. The optional
    provider must be configured explicitly when used outside this project.
    """
    endpoint = os.environ.get("AUDIOCPP_MS_BASE_URL", "").strip().rstrip("/")
    if not endpoint:
        raise RuntimeError("optional provider endpoint is not configured")
    return endpoint
